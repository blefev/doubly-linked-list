...
template <class T> void DLL<T>::prepend(T &data) {
...
template <class T> bool DLL<T>::removeFront() {
  if (!head) {
    return false;
  }
...
template <class T> bool DLL<T>::removeBack() {
  if (!head) {
    return false;
  }
...
template <class T> Node<T> *DLL<T>::search(T target) {
...
template <class T> bool DLL<T>::remove(T target) {
...
template <class T> T DLL<T>::getFront(void) {
  if (!head) {
    return 0;
  }
...
template <class T> T DLL<T>::getBack(void) {
  Node<T> *nodePtr = this->head;
...
template <class T> bool DLL<T>::deleteFromPtr(Node<T> **dblNodePtr) {
  if (!*dblNodePtr) {
    return false;
  }
